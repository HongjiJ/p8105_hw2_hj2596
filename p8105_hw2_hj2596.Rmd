...
The dataset `snp` have `r nrow(unemployment)` observations(rows) and `r ncol(unemployment)` cols, and the variables are `r names(unemployment)`. The years are in the range between `r min(unemployment$year)` and `r max(unemployment$year)`.